...
Similar to Controllability not being able to control any arbitrary system, not all systems can also be completely observable. In most of such cases, a rich set of sensor skirt has be added additionally, such that it improves the output $y$ and from there $\hat{x}$ can be
determined.

Consider a modest example or a discrete system with no input as given below, 
\begin{align}
x_{k+1} &= A x_{k} \\
y_{k} &= C x_{k}
\end{align} 
...
\end{equation}
here the matrix on the RHS is called the Observability matrix $\Omega$, with the system being observable only when $\Omega$ has a full rank of $n$.
...
\subsection{Key points from Linear Algebra}
The linear algebraic equation $\dot{x} = Ax + Bu$ is not only limited for capturing state dynamics. This linear equation is also used as an update equation. Consider update equations for position and velocities as shown in the following. This update equation is a simplified filter used to measure and update the current position of a robot from sensor data.
\begin{align*}
	x_{t+1} &= x_{t} + \dot{x}\cdot\Delta t \\
	y_{t+1} &= y_{t} + \dot{y}\cdot\Delta t \\
	\dot{x} &= \dot{x} + u_{pedal} \\
	\dot{y} &= \dot{y} + u_{steering}
\end{align*}
The above equation as it is a linear algebraic equation can therefore be written in matrix form as shown below:
\begin{align*}
	\vec{x}_{t} = \vec{A}x + \vec{B}u
\end{align*}
where
\begin{align*}
	\vec{x}_{t} &= \begin{bmatrix}
	x_t \\ y_t \\ \dot{x} \\ \dot{y}
	\end{bmatrix} \\
	u &= \begin{bmatrix}
	0 \\ 0 \\ u_{pedal} \\ u_{steering}
	\end{bmatrix} \\
	\vec{A} &= \begin{bmatrix}
	1 & 0 & \Delta t & 0 \\
	0 & 1 & 0 & \Delta t \\
